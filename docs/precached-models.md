# Vertex AI Models Precached List

Last updated: 2026-01-20

This list is automatically scraped from the official Vertex AI documentation and serves as a fallback/precache for the plugin's model discovery.

## Featured Gemini Models

- gemini-3-pro
- gemini-3-flash
- gemini-2.5-flash-image

## Generally Available Gemini Models

- gemini-2.5-pro
- gemini-2.5-flash
- gemini-2.5-flash-image
- gemini-2.5-flash-lite
- gemini-2.5-flash-live-api
- gemini-2.0-flash
- gemini-2.0-flash-lite

## Preview Gemini Models

- gemini-3-pro-preview
- gemini-3-flash-preview
- gemini-3-pro-image

## Gemma Models

- gemma-3n
- gemma-3
- gemma-2
- gemma
- shieldgemma-2
- paligemma
- codegemma
- txgemma
- medgemma
- medsiglip
- t5gemma

## Embeddings Models

- text-embedding-005
- text-embedding-004
- text-multilingual-embedding-002
- multimodalembedding

## Imagen Models

- imagen-4.0-generate-001
- imagen-4.0-fast-generate-001
- imagen-4.0-ultra-generate-001
- imagen-3.0-generate-002
- imagen-3.0-generate-001
- imagen-3.0-fast-generate-001
- imagen-3.0-capability-001

## Preview Imagen Models

- imagen-virtual-try-on-preview
- imagen-product-recontext-preview

## Veo Models

- veo-2.0-generate-001
- veo-3.0-generate-001
- veo-3.0-fast-generate-001
- veo-3.1-generate-001
- veo-3.1-fast-generate-001

## Preview Veo Models

- veo-3.0-generate-preview
- veo-3.0-fast-generate-preview
- veo-3.1-generate-preview
- veo-3.1-fast-generate-preview

## Experimental Veo Models

- veo-2.0-generate-exp